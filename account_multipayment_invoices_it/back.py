...
class account_letras_payment(models.Model):
	...
	usar_termino = fields.Boolean('Usar Termino de Pago',default=True)
		...
		if not self.asiento.id:
			...
			if self.tipo == '2':
				...
				for i in self.factura_ids:
					di = {
						'move_id':t.id,
						'account_id': param.cuenta_letras_fproveedor_me.id if i.divisa.id else param.cuenta_letras_fproveedor_mn.id,
				...
				for i in self.factura_ids:
					di = {
						'move_id':t.id,
						'account_id':param.cuenta_letras_fcliente_me.id if i.divisa.id else param.cuenta_letras_fcliente_mn.id,
						'partner_id':self.partner_id.id,
						'type_document_it':i.tipo_doc.id,
						'nro_comprobante':i.nro_comprobante,
						'name':'CANJE DE LETRAS POR FACTURA',
